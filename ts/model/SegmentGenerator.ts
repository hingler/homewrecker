...
export class SegmentGenerator {
  ...
  static generateSegments(extrude: number, seed?: number) {
    ...
    // 2.5 - 10.0
    const length = 6.0 + random.float(0.0, 1.0) * 5.0;
    // pull int values from this
    const seed_axis = random.integer();
    ...
    const wingCount = Math.floor(3.0 * random.float(0.0, 1.0) + 1.0);
    for (let i = 0; i < wingCount; i++) {
      let placeSegment = false;
      let tries = 0;
      while (!placeSegment && tries < 24) {
        placeSegment = true;
        const index = Math.abs(random.integer() >>> 12) % segmentList.length;
        // pick random parent
        const parent = segmentList[index];
  
        // boost the probability slightly of branching off the ends
        const branchPos = Math.min(1.0, Math.max(random.float(0.0, 1.0) * (1.0 + EDGE_BRANCH_PROBABILITY) - 0.5 * EDGE_BRANCH_PROBABILITY, 0.0));
        ...
        const flip = Math.round(random.float(0.0, 1.0)) * 2 - 1;
        // norm2 is in one of the orthogonal directions
        vec2.scale(norm2, norm2, flip);      
        
        
        const childLength = Math.max(3.5, extrude + 0.5) + random.float(0.0, 1.0) * 4.5;
